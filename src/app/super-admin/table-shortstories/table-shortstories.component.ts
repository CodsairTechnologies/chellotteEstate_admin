...
@Component({
  selector: 'app-table-shortstories',
  standalone: true,
  imports: [TableComponent,
    ...
    DialogModule,
    AvatarModule,
    CommonModule], templateUrl: './table-shortstories.component.html',
  styleUrl: './table-shortstories.component.css'
})
export class TableShortstoriesComponent {
  token: any;
  adminid: any;
  userName: any;
  status: any;
  ...
  arrColumns: any = [
    ...
    { strHeader: "Banner image", strAlign: "center", strKey: "bookcoverimage", field: "bookcoverimage" },

    { strHeader: "Story Name", strAlign: "center", strKey: "booktitle", field: "booktitle" },
    { strHeader: "Author Name", strAlign: "center", strKey: "reviewername", field: "reviewername" },
  ...
  getTableFn() {
    ...
    this.objApiService.handleApiCall('/api/admin/getshortstory/', { id: 'sample' }, (res) => {
      this.Loader = false;

      if (res.response === 'Success' && res.shortstory?.length) {
        res.shortstory.map((obj: { [x: string]: any }, index: number) => {
          obj['imageurl'] = environment.apiUrl + obj['imageurl'];
        });
        this.arrList = res.shortstory.map((obj: any, i: number) => ({ ...obj, slNo: i + 1 }));
 ...
 */
  deleteFn() {
    ...
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/deleteshortstory/', { id: this.ID }, (res) => {
      this.Loader = false;
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/updateshortstorystatus/', { id, status }, (res) => {
      this.Loader = false;
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
  ...
  eventFromTable(objEvent: any) {
    switch (objEvent.strOperation) {
      case 'EDIT_DATA':
        ...
        this.router.navigate(['/super-admin/shortstories'], {
      ...
      case 'SINGLEVIEW_DATA':
        ...
        this.router.navigate(['/super-admin/viewshortstories'], {
          queryParams: { id: id },
        });
  ...
  naviagteTo() {
    this.router.navigateByUrl('/super-admin/shortstories')
  }
