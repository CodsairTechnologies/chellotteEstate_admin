...
@Component({
  ...
  imports: [TableComponent,
    ...
    DialogModule,
    AvatarModule,
    CommonModule], templateUrl: './table-poems.component.html',
  styleUrl: './table-poems.component.css'
})
export class TablePoemsComponent {
  token: any;
  adminid: any;
  userName: any;
  status: any;
  ...
  arrColumns: any = [
    ...
    { strHeader: "Banner", strAlign: "center", strKey: "bannerimage", field: "bannerimage" },
    { strHeader: "Poem Name", strAlign: "center", strKey: "booktitle", field: "booktitle" },
  ...
  getTableFn() {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/getpoems/', { id: 'sample' }, (res) => {
      this.Loader = false;
      if (res.response === 'Success' && res.poems?.length) {
        res.poems.map((obj: { [x: string]: any; }, index: number) => {
          obj['imageurl'] = environment.apiUrl + obj['imageurl'];

        });
        this.arrList = res.poems.map((obj: any, i: number) => ({ ...obj, slNo: i + 1 }));
      }
 ...
 */
  deleteFn() {
    ...
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/deletepoems/', { id: this.ID }, (res) => {
      this.Loader = false;
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/updatepoemsstatus/', { id, status }, (res) => {
      this.Loader = false;
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
