...
token: any;
  ...
  constructor(private formbuilder: FormBuilder, private http: HttpClient, private router: Router, private objApiService: ApiIntegrationService, @Inject(PLATFORM_ID) private platformId: Object) { }
   ...
   */
  getTableFn() {
    this.objApiService.handleApiCall('/api/admin/getbanner/', { id: 'sample' }, (res) => {
      if (res.response === 'Success' && res.banners?.length) {
        res.banners.map((obj: any) => {
          obj['imageurl'] = environment.apiUrl + obj['bannerurl'];

          obj['title'] = obj['title'] && obj['title'].toString().trim() && obj['title'] !== 'null'
            ? obj['title']
            : '--';
            obj['description'] = obj['description'] && obj['description'].toString().trim() && obj['description'] !== 'null'
            ? obj['description']
            : '--';
             obj['link'] = obj['link'] && obj['link'].toString().trim() && obj['link'] !== 'null'
            ? obj['link']
            : '--';
        });
 ...
 */
  ...
  modalHeader: string = 'Add Home Banner';
  openCategoryModal(isEdit: boolean, id?: string, bannerId?: string) {
    this.modalHeader = 'Add Home Banner';

    this.OpenModal = true;
    if (isEdit && id && bannerId) {
      this.id = id;
      this.getBannerByid(this.id, this.bannerId);
    } else {
  ...
  getBannerByid(id: string, bannerId: string) {
    this.modalHeader = 'Edit Home Banner';
    this.id = id;
    this.bannerId = bannerId;
    this.Loader = true;

    this.objApiService.handleApiCall('/api/admin/getbannerbyid/', { id: this.id, banner_id: this.bannerId }, (response) => {
      this.Loader = false;
  ...
  handleBannerOperation() {
    ...
    this.objApiService.handleApiCall('/api/admin/add_editbanner/', formData, (response) => {
      this.Loader = false;
  ...
  if (file) {
    ...
    reader.onload = (e: any) => {
      ...
      img.onload = () => {
        ...
        // if (width < 1280 || height < 480) {   // 16:6 -> 1280×480
        ...
        // ✅ If valid, show preview + patch form
        this.imagePreview = e.target.result;
        this.showImageBox = true;
 ...
 */
  deleteFn() {
    ...
    this.objApiService.handleApiCall('/api/admin/deletebanner/', { banner_id: this.ID }, (res) => {
 ...
 * status change 
 */
  toggleActiveInactive(banner_id: number, status: string) {
    this.objApiService.handleApiCall('/api/admin/updatebannerstatus/', { banner_id, status }, (res) => {
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
  ...
  eventFromTable(objEvent: any) {
    switch (objEvent.strOperation) {
      case 'EDIT_DATA':
        ...
        this.bannerId = objEvent.objElement.bannerId;
        this.getBannerByid(this.ID, this.bannerId);
        break;

      case 'DELETE_DATA':
        this.showDeleteDialog()
        console.log(objEvent.objElement.bannerId)
        this.ID = objEvent.objElement.bannerId;
        break;

      case 'SINGLEVIEW_DATA':
        this.selectedBanner = objEvent.objElement;
      ...
      case 'TOGGLETABLE_DATA':
        ...
        this.toggleActiveInactive(objEvent.objElement.bannerId, newStatus);
