...
@Component({
  ...
  imports: [TableComponent,
    ...
    AvatarModule,
    CommonModule],
  templateUrl: './table-novels.component.html',
  styleUrl: './table-novels.component.css'
})
export class TableNovelsComponent {
  token: any;
  adminid: any;
  userName: any;
  status: any;
  ...
  arrColumns: any = [
    ...
    { strHeader: "Novel Name", strAlign: "center", strKey: "booktitle", field: "booktitle" },
  ...
  getTableFn() {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/getnovels/', { id: 'sample' }, (res) => {
      this.Loader = false;
      if (res.response === 'Success' && res.novel?.length) {
        res.novel.map((obj: { [x: string]: any; }, index: number) => {
          obj['imageurl'] = environment.apiUrl + obj['imageurl'];

        });
        this.arrList = res.novel.map((obj: any, i: number) => ({ ...obj, slNo: i + 1 }));
      } 
 ...
 */
  deleteFn() {
    ...
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/deletenovel/', { id: this.ID }, (res) => {
      this.Loader = false;
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/updatenovelstatus/', { id, status }, (res) => {
      this.Loader = false;
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
  ...
  eventFromTable(objEvent: any) {
    switch (objEvent.strOperation) {
      case 'EDIT_DATA':
        ...
        this.router.navigate(['/super-admin/novels'], {
      ...
      case 'SINGLEVIEW_DATA':
        ...
        this.router.navigate(['/super-admin/viewnovels'], {
          queryParams: { id: id },
        });
  ...
  naviagteTo() {
    this.router.navigateByUrl('/super-admin/novels')
  }
