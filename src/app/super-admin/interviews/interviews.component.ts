 ...
 token: any;
  ...
  ngOnInit(): void {
    ...
    this.InterviewForm = this.formbuilder.group({
      ...
      InterviewContent: ['', Validators.required],
  ...
  getInterviewById(id: string) {
    ...
    this.objApiService.handleApiCall('/api/admin/getinterviewbyid/', { id: this.ID }, (response) => {
      this.Loader = false;

      if (response['response'] === 'Success' && response['interview']) {
        const story = response['interview'];

        this.InterviewForm.patchValue({
          authorName: story.reviewername || '',
          designation: story.reviewerjob || '',
          aboutAuthor: story.aboutreviewer || '',
          InterviewTitle: story.booktitle || '',
          InterviewContent: story.content || ''
        });
        ...
        if (story.reviewerimage) {
          ...
          this.InterviewForm.get('authorImage')?.setValue(true);
        ...
        if (story.bookcoverimage) {
          ...
          this.InterviewForm.get('bannerImage')?.setValue(true);

        }


      } else {
        this.showError(response.message || 'Failed to fetch Interview details.');
      }
    });
  }
...
onCoverUpload(event: any) {
  ...
  reader.onload = (e: any) => {
    ...
    img.onload = () => {
      ...
      this.InterviewForm.patchValue({ bannerImage: file });
      this.InterviewForm.get('bannerImage')?.updateValueAndValidity();
  ...
  clearCoverImage() {
    this.InterviewForm.patchValue({ bannerImage: null });
...
previewAuthorImage(file: File) {
  ...
  reader.readAsDataURL(file);
  this.InterviewForm.patchValue({ authorImage: file });
  this.InterviewForm.get('authorImage')?.updateValueAndValidity();
}
...
clearAuthorImage() {
  ...
  this.InterviewForm.patchValue({ authorImage: null });
}
  ...
  //     this.InterviewForm.get('bannerImage')?.setValue(file);
  //     this.InterviewForm.get('bannerImage')?.markAsTouched();
  //   }
  // }
  // clearBannerImage() {
  //   this.bannerPreview = '';
  ...
  //   this.bannerFileType = '';
  //   this.InterviewForm.get('bannerImage')?.setValue(null);
  //   this.InterviewForm.get('bannerImage')?.markAsTouched();
  // }
  ...
  handleInterviewsubmit() {
    ...
    formData.append('reviewername', this.InterviewForm.get('authorName')?.value);
    formData.append('reviewerjob', this.InterviewForm.get('designation')?.value);
    formData.append('aboutreviewer', this.InterviewForm.get('aboutAuthor')?.value);
    formData.append('booktitle', this.InterviewForm.get('InterviewTitle')?.value);
    formData.append('content', this.InterviewForm.get('InterviewContent')?.value);
    formData.append('createdId', this.adminid); // optional

    // Conditionally append files
    const authorImage = this.InterviewForm.get('authorImage')?.value;
    if (authorImage instanceof File) {
      formData.append('reviewerimage', authorImage);
    }

    const bannerImage = this.InterviewForm.get('bannerImage')?.value;
    ...
    if (this.ID) {
      formData.append('interview_id', this.ID);
    }

    this.Loader = true;

    this.objApiService.handleApiCall('/api/admin/addeditinterview/', formData, (response) => {
      this.Loader = false;
      if (response['response'] === 'Success') {
        this.showSuccess(response.message);
        this.InterviewForm.reset();
                this.router.navigateByUrl('/super-admin/listinterviews')
  ...
  onFileUpload(event: any) {
    ...
    if (file) {
      ...
      reader.readAsDataURL(file);
      this.InterviewForm.patchValue({ image: file });
      this.InterviewForm.get('image')?.updateValueAndValidity();
    }
  }
  ...
  clearCard() {
    ...
    this.InterviewForm.patchValue({ image: null });
    this.InterviewForm.get('image')?.updateValueAndValidity();
  }
