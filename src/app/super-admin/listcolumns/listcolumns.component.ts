...
@Component({
  ...
  templateUrl: './listcolumns.component.html',
  styleUrl: './listcolumns.component.css'
})
export class ListcolumnsComponent {
  ...
  arrColumns: any = [
    ...
    { strHeader: "Column Name", strAlign: "center", strKey: "booktitle", field: "booktitle" },
  ...
  getTableFn() {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/getcolumn/', { id: 'sample' }, (res) => {
      this.Loader = false;
      if (res.response === 'Success' && res.column?.length) {
        res.column.map((obj: { [x: string]: any; }, index: number) => {
          obj['imageurl'] = environment.apiUrl + obj['imageurl'];

        });
        this.arrList = res.column.map((obj: any, i: number) => ({ ...obj, slNo: i + 1 }));
      }
 ...
 */
  deleteFn() {
    ...
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/deletecolumn/', { id: this.ID }, (res) => {
      this.Loader = false;
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/updatecolumnstatus/', { id, status }, (res) => {
      this.Loader = false;
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
  ...
  eventFromTable(objEvent: any) {
    switch (objEvent.strOperation) {
      case 'EDIT_DATA':
        ...
        this.router.navigate(['/super-admin/columns'], {
      ...
      case 'SINGLEVIEW_DATA':
        ...
        this.router.navigate(['/super-admin/viewcolumns'], {
          queryParams: { id: id },
        });
  ...
  naviagteTo() {
    this.router.navigateByUrl('/super-admin/columns')
  }
