...
export class NovelsComponent {
 token: any;
  ...
  ngOnInit(): void {
    ...
    this.NovelsForm = this.formbuilder.group({
      authorName: ['', Validators.required],
      designation: [''],
      aboutAuthor: [''],
      authorImage: [null],
      NovelsTitle: ['', Validators.required],
      NovelsContent: ['', Validators.required],
  ...
  getNovelsById(id: string) {
    this.ID = id;
    this.Loader = true;

    this.objApiService.handleApiCall('/api/admin/getnovelbyid/', { id: this.ID }, (response) => {
      this.Loader = false;

      if (response['response'] === 'Success' && response['novel']) {
        const story = response['novel'];

        this.NovelsForm.patchValue({
          authorName: story.reviewername || '',
          designation: story.reviewerjob || '',
          aboutAuthor: story.aboutreviewer || '',
          NovelsTitle: story.booktitle || '',
          NovelsContent: story.content || ''
        });
        ...
        if (story.reviewerimage) {
          ...
          this.NovelsForm.get('authorImage')?.setValue(true);
        ...
        if (story.bookcoverimage) {
          ...
          this.NovelsForm.get('bannerImage')?.setValue(true);

        }


      } else {
        this.showError(response.message || 'Failed to fetch Novels details.');
      }
    });
  }
...
onCoverUpload(event: any) {
  ...
  reader.onload = (e: any) => {
    ...
    img.onload = () => {
      ...
      this.NovelsForm.patchValue({ bannerImage: file });
      this.NovelsForm.get('bannerImage')?.updateValueAndValidity();
  ...
  clearCoverImage() {
    this.NovelsForm.patchValue({ bannerImage: null });
...
previewAuthorImage(file: File) {
  ...
  reader.readAsDataURL(file);
  this.NovelsForm.patchValue({ authorImage: file });
  this.NovelsForm.get('authorImage')?.updateValueAndValidity();
}
...
clearAuthorImage() {
  ...
  this.NovelsForm.patchValue({ authorImage: null });
}
  ...
  //     this.NovelsForm.get('bannerImage')?.setValue(file);
  //     this.NovelsForm.get('bannerImage')?.markAsTouched();
  //   }
  // }
  // clearBannerImage() {
  //   this.bannerPreview = '';
  ...
  //   this.bannerFileType = '';
  //   this.NovelsForm.get('bannerImage')?.setValue(null);
  //   this.NovelsForm.get('bannerImage')?.markAsTouched();
  // }

  // Function to handle Add/Edit operation
  handleNovelsubmit() {
    if (this.NovelsForm.invalid) {
      this.NovelsForm.markAllAsTouched();
      return;
    }

    const formData = new FormData();
    formData.append('reviewername', this.NovelsForm.get('authorName')?.value);
    formData.append('reviewerjob', this.NovelsForm.get('designation')?.value);
    formData.append('aboutreviewer', this.NovelsForm.get('aboutAuthor')?.value);
    formData.append('booktitle', this.NovelsForm.get('NovelsTitle')?.value);
    formData.append('content', this.NovelsForm.get('NovelsContent')?.value);
    formData.append('createdId', this.adminid); // optional

    // Conditionally append files
    const authorImage = this.NovelsForm.get('authorImage')?.value;
    if (authorImage instanceof File) {
      formData.append('reviewerimage', authorImage);
    }

    const bannerImage = this.NovelsForm.get('bannerImage')?.value;
    ...
    this.objApiService.handleApiCall('/api/admin/addeditnovel/', formData, (response) => {
      this.Loader = false;
      if (response['response'] === 'Success') {
        this.showSuccess(response.message);
        this.NovelsForm.reset();
                this.router.navigateByUrl('/super-admin/listnovels')
  ...
  onFileUpload(event: any) {
    ...
    if (file) {
      ...
      reader.readAsDataURL(file);
      this.NovelsForm.patchValue({ image: file });
      this.NovelsForm.get('image')?.updateValueAndValidity();
    }
  }
  ...
  clearCard() {
    ...
    this.NovelsForm.patchValue({ image: null });
    this.NovelsForm.get('image')?.updateValueAndValidity();
  }
