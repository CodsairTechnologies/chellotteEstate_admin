...
@Component({
  ...
  imports: [TableComponent,
    ...
    AvatarModule,
    CommonModule],
  templateUrl: './letusreadwbanner.component.html',
  styleUrl: './letusreadwbanner.component.css'
})
export class LetusreadwbannerComponent {
  token: any;
  adminid: any;
  userName: any;
  status: any;
   ...
   */
getTableFn() {
  this.objApiService.handleApiCall('/api/admin/getletusreadbanners/', { id: 'sample' }, (res) => {
 ...
 */
  ...
  modalHeader: string = 'Add Readers Forum Banner';
  openCategoryModal(isEdit: boolean, id?: string) {
    this.modalHeader = 'Add Readers Forum Banner';
  ...
  getCategoryById(id: string) {
    this.modalHeader = 'Edit Letusread Banner';
    this.id = id;
    this.Loader = true;

    this.objApiService.handleApiCall('/api/admin/getletusreadbannerbyid/', { id: this.id }, (response) => {
      this.Loader = false;
  ...
  handleCategoryOperation() {
    ...
    this.objApiService.handleApiCall('/api/admin/addeditletusreadbanner/', formData, (response) => {
      this.Loader = false;
 ...
 */
  deleteFn() {
    ...
    this.objApiService.handleApiCall('/api/admin/deleteletusreadbanner/', { id: this.ID }, (res) => {
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.objApiService.handleApiCall('/api/admin/updateletusreadbannerstatus/', { id, status }, (res) => {
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
