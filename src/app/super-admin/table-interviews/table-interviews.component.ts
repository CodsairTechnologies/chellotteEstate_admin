...
@Component({
  ...
  imports: [TableComponent,
    ...
    AvatarModule,
    CommonModule],
  templateUrl: './table-interviews.component.html',
  styleUrl: './table-interviews.component.css'
})
export class TableInterviewsComponent {
token: any;
  adminid: any;
  userName: any;
  status: any;
  ...
  arrColumns: any = [
    ...
    { strHeader: "Interview Name", strAlign: "center", strKey: "booktitle", field: "booktitle" },
    { strHeader: "Interviewer Title", strAlign: "center", strKey: "reviewername", field: "reviewername" },
  ...
  getTableFn() {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/getinterviews/', { id: 'sample' }, (res) => {
      this.Loader = false;
      if (res.response === 'Success' && res.interviews?.length) {
        res.interviews.map((obj: { [x: string]: any; }, index: number) => {
          obj['imageurl'] = environment.apiUrl + obj['imageurl'];

        });
        this.arrList = res.interviews.map((obj: any, i: number) => ({ ...obj, slNo: i + 1 }));
      }
 ...
 */
  deleteFn() {
    ...
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/deleteinterview/', { id: this.ID }, (res) => {
      this.Loader = false;
 ...
 */
  toggleActiveInactive(id: number, status: string) {
    this.Loader = true;
    this.objApiService.handleApiCall('/api/admin/updateinterviewstatus/', { id, status }, (res) => {
      this.Loader = false;
      if (res.response === 'Success') {
        this.showSuccess(res.message);
        this.getTableFn();
  ...
  eventFromTable(objEvent: any) {
    switch (objEvent.strOperation) {
      case 'EDIT_DATA':
        ...
        this.router.navigate(['/super-admin/interviews'], {
      ...
      case 'SINGLEVIEW_DATA':
        ...
        this.router.navigate(['/super-admin/viewinterviews'], {
          queryParams: { id: id },
        });
  ...
  naviagteTo() {
    this.router.navigateByUrl('/super-admin/interviews')
  }
